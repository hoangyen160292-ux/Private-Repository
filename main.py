...
def get_actual_model():
    """诊断函数：确认新 Key 的实时权限"""
    if not AI_API_KEY:
        print(">>> 错误：未检测到 AI_API_KEY，请检查 GitHub Secrets 配置。")
        return None
    ...
    try:
        ...
        data = res.json()
        if "error" in data:
            print(f">>> API 异常: {data['error'].get('message', '未知错误')}")
            return None
